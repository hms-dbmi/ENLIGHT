...
class ABMIL(nn.Module):
    ...
    def forward_to_loss(self, crterion, x, wslabel):
        slide_preds = self.forward(x)
        loss = crterion(slide_preds, wslabel)
        return loss, {}
...
class MABMIL(nn.Module):
    def __init__(self, conf, D_att=128, N_att=1, droprate=0):
        super(MABMIL, self).__init__()
        self.D_inner = conf.D_inner
        self.D_atthead = getattr(conf, 'D_att', D_att)
        self.N_att = getattr(conf, 'N_att', N_att)
        self.droprate = getattr(conf, 'droprate', droprate)
        self.K_feat = len(conf.D_feat)
        assert isinstance(conf.D_feat, dict)

        self.dimreduction = nn.ModuleList()
        self.feat_sources = []
        for feat, in_dim in conf.D_feat.items():
            self.dimreduction.append(DimReduction(in_dim, self.D_inner))
            self.feat_sources.append(feat)
        
        
        self.attention = Attention_Gated(self.D_inner, self.D_atthead, self.N_att)
        self.classifier = Classifier_1fc(self.D_inner, conf.n_class, self.droprate)

    def forward(self, xdict): ## xdict: (N,D1)...(N,Dk)
        inner_feats = []
        for source, head in zip(self.feat_sources, self.dimreduction):
            x = xdict[source]
            if len(x.shape) == 3:
                x = x[0]
            med_feat = head(x)
            inner_feats.append(med_feat)
        inner_feats = torch.stack(inner_feats, dim=0) #(k, N, D)
        inner_feats = inner_feats.reshape(-1,  inner_feats.shape[-1]) #(k*N, D)  N x D
        A = self.attention(inner_feats)  ## (head, k*N)
        A = F.softmax(A, dim=1)  # softmax over k*N
        self.A = A
        # A = A.reshape(self.D_atthead, self.K_feat, -1)## (head, k, N)
        afeat = torch.mm(A, inner_feats) ## (head, D) #Same weights shared by the dimension
        outputs = self.classifier(afeat)
        return outputs
